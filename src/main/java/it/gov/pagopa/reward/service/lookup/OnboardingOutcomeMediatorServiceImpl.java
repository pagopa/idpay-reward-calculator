...
@Service
@Slf4j
public class OnboardingOutcomeMediatorServiceImpl extends BaseKafkaConsumer<EvaluationDTO, EvaluationDTO> implements
    ...
    protected void notifyError(Message<String> message, Throwable e) {
        rewardErrorNotifierService.notifyTransactionEvaluation(message, "[HPAN_INITIATIVE_OP] An error occurred evaluating hpan update", false, e);
    }
    ...
    protected Consumer<Throwable> onDeserializationError(Message<String> message) {
        return e -> rewardErrorNotifierService.notifyTransactionEvaluation(message, "[HPAN_INITIATIVE_OP] Unexpected JSON", true, e);
    }

    @Override
    protected Mono<EvaluationDTO> execute(EvaluationDTO payload, Message<String> message, Map<String, Object> ctx) {
//        LocalDateTime evaluationDate = LocalDateTime.now();
        ...
        return rewardContextHolderService.getInitiativeConfig(payload.initiativeId())
                .switchIfEmpty(Mono.error(new IllegalArgumentException("Cannot find initiative having id %s".formatted(payload.initiativeId()))))
                .flatMap(initiativeConfig -> {
                    ...
                    if (InitiativeGeneralDTO.BeneficiaryTypeEnum.NF.equals(bType)) {
                        ...
                        if (payload.familyId() != null && !payload.familyId().isBlank()) {
                            return userInitiativeCountersRepository.createIfNotExists(payload.familyId(), bType, payload.initiativeId())
                                    .thenReturn(payload);
                        }
                        return onboardingFamiliesRepository.findByMemberIdsInAndInitiativeId(payload.userId(), payload.initiativeId())
                                .collectSortedList(COMPARATOR_FAMILIES_CREATE_DATE_DESC)
                                .flatMap(families -> {
                                    if (families == null || families.isEmpty()) {
                                        return Mono.just(payload);
                                    }
                                    return userInitiativeCountersRepository.createIfNotExists(families.get(0).getFamilyId(), bType, payload.initiativeId())
                                            .thenReturn(payload);
                                });
                    } else {
                        // person-level counters
                        return userInitiativeCountersRepository.createIfNotExists(payload.userId(), bType, payload.initiativeId())
                                .thenReturn(payload);
                    }
                })
                .onErrorResume(e -> {
                    log.error("[HPAN_INITIATIVE_OP] Error while creating user initiative counter for event {}", payload, e);
